...
public class GA {
	...
	public static void main(String[] args)  {
		new GA().doIt(Integer.parseInt("55"));
	...
	static int poolSize = 40;	// Must be even
        ...
        //Testes
        double coScore = 0.0;
        double newMScore = 0.0;
        int numMut = 0;
        int numCo = 0;
        double totscore = 0.0;
        double mediagen = 0.0;
        static double crossChangeRate = 0.001;
        static double mutChangeRate = 0.0002;
		...
		while(true) {
			...
			for(int x=pool.size()-1;x>=0;x-=2) {
				...
				if(n1.crossOver(n2)){
                                ...
                                coScore += n1.newScoreChromo(target) + n2.newScoreChromo(target) - (n1.score + n2.score);
                                
                                
				if(n1.mutate()){
                                    numMut++;
                                    newMScore += n1.newScoreChromo(target) - n1.score;
                                }
                                
				
                                if(n2.mutate()){
                                    numMut++;
                                    newMScore += n2.newScoreChromo(target) - n2.score;
                                }
				...
				if (n1.total == target && n1.isValid()) { System.out.println("Generations: " + gen + "  Solution: " + n1.decodeChromo()+ "\n Media: "+ mediagen ); return; }
				if (n2.total == target && n2.isValid()) { System.out.println("Generations: " + gen + "  Solution: " + n2.decodeChromo()+ "\n Media: "+ mediagen ); return; }
			...
			pool.addAll(newPool);
                         mediagen = totscore / poolSize;
                        //Recalcula multRate e crossRate
                        if(coScore/numCo > newMScore/numMut){
                            crossRate += crossChangeRate;
                            mutRate -= mutChangeRate;
	...
	private static class Chomosone {
		...
		public final void scoreChromo(int target) {
			...
			score = (double)1 / (target - total);
                 ...
                 public double newScoreChromo(int target) {
			total = addUp();
			if (total == target) return 0;
			return (double)1 / (target - total);
		}
		
		// Crossover bits
		public boolean crossOver(Chomosone other) {

			// Should we cross over?
			if (rand.nextDouble() > crossRate) return false;
